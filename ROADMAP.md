...
**Still to come on Supabase:** checkout via Edge Functions (Phase 3c) — there is no
...
| Capability | Free | Pro (one-time) | Live? |
|---|:---:|:---:|:---:|
| Core queue + auto-pick | ✅ | ✅ | ✅ |
| Courts | 2 | 6 | ✅ 3a |
| Players per room | 10 | unlimited | ✅ 3a |
| Saved rooms | 2 | unlimited | ✅ 3a |
| Persistent player roster (reuse weekly) | — | ✅ | Phase 4 |
| Match history & score tracking | — | ✅ | Phase 4 |
| Stats (games played, fairness, win/loss) | basic | full | Phase 4 |
| Courtside "TV mode" big-screen display | — | ✅ | Phase 4 |
| CSV export | — | ✅ | Phase 4 |
| Custom skill labels / club name | — | ✅ | Phase 4 |

**The three enforced rows changed on the way in** _(2026-09-12)_. This table originally proposed
2 courts / ~16 players / 1 saved session; what shipped is **2 / 10 / 2** — tighter on players, looser
on rooms. Tighter because §3's own argument says a one-time price means free users must stay cheap to
serve; looser because "1 saved room" walls a returning organizer on their second ever tap, and
"Create a room" is the landing page's primary CTA. Pro's court number is **6, not "unlimited"**:
`sessions.courts` carries a `check (courts between 1 and 6)`, so anything higher would be a promise
the table itself rejects.

_(Still a starting point — validate against how real clubs use it. They live in one `plan_limits()`
function, so re-tuning is a one-line migration.)_
...
**Shipped to production the same day** and re-verified against the live project, as Phase 2 was.
All five existing rooms resolve their limits — which matters more than it sounds, because
`getSessionByCode` now *throws* if `session_limits()` returns no row, so a broken predicate would
have taken every room page down rather than degrading quietly:

| room | owner | resolves to | |
|---|---|---|---|
| 6 courts | ownerless | `pro`, max 6 | the carve-out working on real data |
| 3 courts | owned | `free`, max 2 | grandfathered |
| 2 courts | owned | `free`, max 2 | already compliant |
| 3 courts | owned | `free`, max 2 | grandfathered |
| 6 courts | owned | `free`, max 2 | grandfathered |

Three of the five sit above their cap and keep every court, exactly as intended. The direct
`PATCH {"courts":5}` bypass was confirmed refused (42501) and the 11th player refused (23514) with
the human-readable message, both against a real room over HTTP rather than through the test client.
...
- **Landing page** polish (screenshots, FAQ), proper metadata/OG, favicon, real name. _(The hero landed in Phase 2.6 and `/pricing` in Phase 3a — don't re-scope either here; what's missing is the marketing around them.)_
...
- **2026-09-12** — **Phase 3a: entitlements and server-enforced free-tier limits.** The app now has
  ...
  refused. New: `app/pricing/page.tsx`, `app/components/LimitNote.tsx`. **Shipped to production the
  same day** and re-verified there: all five existing rooms resolve their limits (which matters,
  since `getSessionByCode` now throws if `session_limits()` returns nothing — a broken predicate
  would have taken every room page down), the one ownerless room resolves to `pro` with its 6 courts
  intact, and three owned rooms sit grandfathered above the new cap. **Nobody was grandfathered into
  Pro, the author included** — deliberately, so the limits get tested by living under them; see §7.
  **Deliberately not done:** anything that takes money, and publishing `entitlements` to Realtime.
  **Unrelated but discovered:** local ports moved off the CLI's `5432x` defaults to `5532x` so this
  stack can coexist with another local Supabase project (local only — CI and production are
